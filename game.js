...
{
    const selectedPlayerNumber = gameSettings.vampireNumber + gameSettings.priestNumber;
    ...
    for(let i=0;i<gameSettings.vampireNumber;i++)
    {
        ...
        playerListWithRole.push({id:shufflePlayerList[selectedIndex].id,name:shufflePlayerList[selectedIndex].name,role:2});
        selectedIndex++;
    }

    for(let i=0;i<gameSettings.priestNumber;i++)
    {
        playerListWithRole.push({id:shufflePlayerList[selectedIndex].id,name:shufflePlayerList[selectedIndex].name,role:3});
        selectedIndex++;
    }
    for(let i = selectedIndex;i<shufflePlayerList.length;i++)
    {
        playerListWithRole.push({id:shufflePlayerList[i].id,name:shufflePlayerList[i].name,role:1});
    }
